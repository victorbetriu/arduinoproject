...
void leds() {
  ...
  //int blinking = map(cm, 0, 300, 1, 255);
  if (millis() % 500 == 0) {
    if (cm > 50) { //cada 1/2ms...
      //Serial.println("good");
      analogWrite(ledBe,  255);
      analogWrite(ledMal, 0);
    }

    if (cm < 50) { //cada 1/2ms...
      //Serial.println("bad");
      analogWrite(ledMal,  255);
      analogWrite(ledBe,  0);
    }
    /*
      if (millis() % blinking == 0) {
      analogWrite(ledBe,  255);
      analogWrite(ledMal, 0);
      }

      if (millis() % blinking == blinking / 2) {
      analogWrite(ledMal,  255);
      analogWrite(ledBe,  0);
      }
  ...
  void Touch() {
    if (millis() % 500) {
      ...
      long total2 = cs_4_13.capacitiveSensor(30);
      ...
      if (total1 > 600 && total2 > 600) {
        analogWrite(ledMal, 100);
        analogWrite(ledBe, 0);
      } else if (total1 > 1000 && total2 > 1000) {
        analogWrite(ledMal, 0);
        analogWrite(ledBe, 100);
      } else if (total1 > 1200 && total2 > 1200) {
        analogWrite(ledMal, 0);
        analogWrite(ledBe, 255);
      }
