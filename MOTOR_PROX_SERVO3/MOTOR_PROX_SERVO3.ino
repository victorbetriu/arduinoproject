...
void Sensor() {
  ...
  if (millis() % 5 == 0) {
    ...
    pos += inc;
    Serial.println(pos);
    if (pos > 170 || pos < 10) { //the servo arribes at the end of one cycle
      inc = -inc;
      Motor();
    }
...
void Motor() {
  ...
  int diferencia = moviments - Po; //choose direction
  if (diferencia > 0) {
    dir = 1;
  } if (diferencia <= 0) {
    dir = 0;
  ...
  int desplacament = abs(diferencia); //moves the specific distance
